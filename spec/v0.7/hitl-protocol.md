...
| `submitted_by` | object | REQUIRED | Identity of the human who triggered the action on the messaging platform. |
...
The `submit_url` MAY point to the same endpoint path as the browser-based respond endpoint (`/reviews/{caseId}/respond`). The service distinguishes between browser submissions (authenticated via `?token=` query parameter) and agent submissions (authenticated via `Authorization: Bearer` header) based on the authentication mechanism used.
...
**Reduced verification context.** When a human submits via the review page, the service's browser context provides CSP, CORS, and same-origin protections. Inline submissions bypass these protections — the agent submits on behalf of the human based on a messaging platform callback.
...
3. **Audit trail.** Every inline submission MUST include `submitted_via` and `submitted_by`. Services SHOULD log these fields and MAY apply different risk policies based on submission channel.
4. **Identity verification.** Services MAY cross-reference `submitted_by.platform_user_id` with a pre-registered identity to verify that the correct human responded.
...
```json
{
  ...
  "properties": {
    ...
    "submit_url": {
      "type": "string",
      "format": "uri",
      "description": "API endpoint for agent inline submissions on behalf of the human (Section 7.5). MUST be HTTPS."
    },
    "submit_token": {
      "type": "string",
      "description": "Opaque bearer token for authenticating agent submissions via submit_url. MUST be present when submit_url is present."
    },
    "inline_actions": {
      "type": "array",
      "items": { "type": "string" },
      "description": "Actions permitted via submit_url. When omitted and submit_url is present, all actions for the review type are permitted."
    },
    "surface": {
      "type": "object",
      "properties": {
